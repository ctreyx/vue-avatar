...
onMounted(() => {
  window.addEventListener('message', saveAvatar)
})

onUnmounted(() => {
  window.removeEventListener('message', saveAvatar)
})

//向父窗口发送消息，保存头像
const saveAvatar = (event: MessageEvent) => {
  let { namespace, action, data } = event.data
  ...
  if (action === 'save-avatar') {
    ...
    // let avatarEleStr = avatarEle!.outerHTML

    let avatarEleStr = colorAvatarRef.value?.getSvgContent() as string

    window?.parent?.postMessage(
      { action: 'update-avatar', namespace: 'tx', data: avatarEleStr },
      '*'
    )
  }

  if (action === 'init-avatar') {
    colorAvatarRef.value?.setSvgContent(data)
  }
}
